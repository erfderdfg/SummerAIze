...
const summaryProvider_1 = require("./summaryProvider");
const aiService_1 = require("./aiService");
function activate(context) {
    try {
        console.log('AI Conversation Summarizer is now active!');
        const aiService = new aiService_1.AIService();
        const summaryProvider = new summaryProvider_1.SummaryProvider(context, aiService);
        ...
        const selectionChangeListener = vscode.window.onDidChangeTextEditorSelection(async (event) => {
            try {
                ...
                const autoSummarize = config.get('autoSummarize', false);
                if (!autoSummarize)
                    return;
        ...
        context.subscriptions.push(refreshCommand, summarizeTextCommand, clearSummariesCommand, exportSummariesCommand, selectionChangeListener);
    }
